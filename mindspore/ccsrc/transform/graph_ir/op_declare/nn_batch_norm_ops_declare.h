...
DECLARE_OP_ADAPTER(BatchNorm)//将BatchNorm收录后与标准进行比较，进行空间调整，用input_map_为key储存
DECLARE_OP_USE_OUTPUT(BatchNorm)//将BatchNorm收录后与标准进行比较，进行空间调整，用output_map_为key储存
