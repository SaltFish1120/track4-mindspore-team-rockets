...
DECLARE_OP_ADAPTER(BiasAddGrad)//函数功能为将BiasAddGrad的类型与标准进行对比，后进行空间调整，并用指针input_map_为key存储相应内容
DECLARE_OP_USE_OUTPUT(BiasAddGrad)//函数功能为将BiasAddGrad的类型与标准进行对比，后进行空间调整，并用指针output_map_为key存储相应内容

DECLARE_OP_ADAPTER(Conv2D)
DECLARE_OP_USE_ENUM(Conv2D)//函数功能为将Conv2D的类型与标准进行对比，后进行空间调整，并用指针enum_map_为key存储相应内容
DECLARE_OP_USE_OUTPUT(Conv2D)

DECLARE_OP_ADAPTER(Conv2DBackpropInputD)
DECLARE_OP_USE_ENUM(Conv2DBackpropInputD)
DECLARE_OP_USE_INPUT_ATTR(Conv2DBackpropInputD)////函数功能为将Conv2DBackpropInputD的类型与标准进行对比，后进行空间调整，并用指针input_attr_map_为key存储相应内容
