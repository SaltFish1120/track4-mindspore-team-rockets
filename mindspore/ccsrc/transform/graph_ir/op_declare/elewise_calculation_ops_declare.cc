...
INPUT_MAP(Assign) = {{1, INPUT_DESC(ref)}, {2, INPUT_DESC(value)}};/*
  将value和ref处理并存入对应INPUT_DESC结构体的相应变量中
  将value和ref内容转为字符串变量并存储至结构体的name变量中
  引用Operator空间并将指针所指的类转为INPUT_DESC结构体
将Assign的类型与标准进行对比，后进行空间调整，并用指针input_map_为key存储相应内容
*/
ATTR_MAP(Assign) = EMPTY_ATTR_MAP;//将Assign与标准进行比较，使原以attr_map_为储存信息的key变为空
OUTPUT_MAP(Assign) = {{0, OUTPUT_DESC(ref)}};/*
  将ref处理并存入对应OUTPUT_DESC结构体的相应变量中
  将ref内容转为字符串变量并存储至结构体的name变量中
  引用Operator空间并将指针所指的类转为OUTPUT_DESC结构体
将Assign的类型与标准进行对比，后进行空间调整，并用指针output_map_为key存储相应内容
*/
REG_ADPT_DESC(Assign, prim::kPrimAssign->name(), ADPT_DESC(Assign))/*
  将Assign处理并存入对应ADPT_DESC结构体的相应变量中
  将Assign内容转为字符串变量并存储至结构体的name变量中
  引用Operator空间并将指针所指的类转为ADPT_DESC结构体
  
  再将Assign处理并存入对应REG_ADPT_DESC结构体的相应变量中
  将Assign内容转为字符串变量并存储至REG_ADPT_DESC的结构体的name变量中
