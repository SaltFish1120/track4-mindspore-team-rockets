...
将TensorScatterUpdate的类型与标准进行对比，后进行空间调整，并用指针input_map_为key存储相应内容
*/
ATTR_MAP(TensorScatterUpdate) = EMPTY_ATTR_MAP;//将TensorScatterUpdate的类型与标准进行对比，后进行空间调整，使原attr_map_为储存相应内容的key为空
OUTPUT_MAP(TensorScatterUpdate) = {{0, OUTPUT_DESC(y)}};/*
  将y处理并存入对应OUTPUT_DESC结构体的相应变量中
  将y内容转为字符串变量并存储至结构体的name变量中
  引用Operator空间并将指针所指的类转为INPUT_DESC结构体
将TensorScatterUpdate的类型与标准进行对比，后进行空间调整，并用指针output_map_为key存储相应内容
*/
REG_ADPT_DESC(TensorScatterUpdate, kNameTensorScatterUpdate, ADPT_DESC(TensorScatterUpdate))/*
  将TensorScatterUpdate处理并存入对应ADPT_DESC结构体的相应变量中
  将TensorScatterUpdate内容转为字符串变量并存储至结构体的name变量中
  引用Operator空间并将指针所指的类转为ADPT_DESC结构体
  
  再将TensorScatterUpdate处理并存入对应REG_ADPT_DESC结构体的相应变量中
  将TensorScatterUpdate内容转为字符串变量并存储至REG_ADPT_DESC的结构体的name变量中
