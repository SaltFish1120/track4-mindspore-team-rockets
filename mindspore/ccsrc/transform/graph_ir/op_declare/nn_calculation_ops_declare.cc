...
将收录内容的类型与标准进行对比，后进行空间调整，并用指针input_attr_map_为key存储相应内容
*/
ATTR_MAP(Conv2DBackpropInputD) = {
  {"pad_list", ATTR_DESC(pads, AnyTraits<std::vector<int64_t>>(), AnyTraits<std::vector<int64_t>>())},/*
  将name变量pads处理并存入对应AttrDesc结构体的相应变量中
  将name变量内容转为字符串变量并存储至结构体的name变量中
  引用Operator空间并将指针所指的类转为AttrDesc结构体
...
将收录内容的类型与标准进行对比，后进行空间调整，并用指针attr_map_为key存储相应内容
*/
...
  将name变量y处理并存入对应OutputDesc结构体的相应变量中
  将name变量内容转为字符串变量并存储至结构体的name变量中
  引用Operator空间并将指针所指的类转为OutputDesc结构体

将收录内容的类型与标准进行对比，后进行空间调整，并用指针output_map_为key存储相应内容
*/
REG_ADPT_DESC(Conv2DBackpropInputD, prim::kPrimConv2DBackpropInput->name(), ADPT_DESC(Conv2DBackpropInputD))/*
  将Conv2DBackpropInputD处理并存入对应ADPT_DESC结构体的相应变量中
  将Conv2DBackpropInputD内容转为字符串变量并存储至结构体的name变量中
  引用Operator空间并将指针所指的类转为ADPT_DESC结构体
  
  再将Conv2DBackpropInputD处理并存入对应REG_ADPT_DESC结构体的相应变量中
  将Conv2DBackpropInputD内容转为字符串变量并存储至REG_ADPT_DESC的结构体的name变量中
