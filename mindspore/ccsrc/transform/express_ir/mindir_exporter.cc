...
#include <map> //STL映射容器
#include <memory> //空间配置器
#include <utility> //STL通用模板类
#include <algorithm> //STL通用算法
#include <functional> //STL定义运算函数（代替运算符）
#include <fstream> //STL中对文件操作的合集

#include "utils/hash_map.h" //在当前路径下引入头文件，下面同理
#include "ir/tensor.h"
...
#ifndef MINDIR_EXPORT_TENSOR_LAYOUT_CLIP //防止重复定义，如果当前宏是否被定义，若没有则定义，若已被定义，执行后面的语句
#include "frontend/parallel/tensor_layout/tensor_layout.h"
#endif //ifndef条件命令结束标志
#include "abstract/abstract_function.h"

namespace mindspore { //创建一个名为mindspore的空间
using FloatPtr = std::shared_ptr<Float>; //引入使用Float空间
using IntPtr = std::shared_ptr<Int>; //引入使用Int空间
using UIntPtr = std::shared_ptr<UInt>; //引入使用UInt空间
using ModelProtoPtr = std::shared_ptr<mind_ir::ModelProto>; //引入使用ModelProto空间

// anf type to mindir type map 将数据类型转为mindir类型，以下同理
...
class IrExportBuilder; //IrExport生成器类
using IrExportBuilderPtr = std::shared_ptr<IrExportBuilder>; //引入使用IrExportBuilder空间

class IrExporter {
 public:
  explicit IrExporter(IrExportBuilderPtr builder) : builder_(std::move(builder)) {} //避免IrExporter隐式转换为类类型
...
using IrExporterPtr = std::shared_ptr<IrExporter>; //引入使用IrExporterr空间

class IrExportBuilder {
 public:
  IrExportBuilder() : model_(std::make_shared<mind_ir::ModelProto>()) {}
  ~IrExportBuilder() = default; //默认构造函数
  std::string GetProtoString() const;
  void BuildModelInfo();
  bool BuildModel(const FuncGraphPtr &func_graph);
  ModelProtoPtr Model() { return model_; }

#ifndef MINDIR_EXPORT_TENSOR_LAYOUT_CLIP //判断当前宏是否被定义
  void BuildLayout(const FuncGraphPtr &func_graph);
#endif
//
  bool BuildFuncGraph(const FuncGraphPtr &func_graph, mind_ir::GraphProto *const graph_proto); //图表
  bool BuildFuncGraphAttrs(const FuncGraphPtr &func_graph, mind_ir::GraphProto *const graph_proto); //图表属性
  bool BuildParameters(const FuncGraphPtr &func_graph, mind_ir::GraphProto *const graph_proto); //参数
  bool BuildNodes(const FuncGraphPtr &func_graph, mind_ir::GraphProto *const graph_proto); //节点
  bool BuildOutput(const CNodePtr &node, mind_ir::GraphProto *const graph_proto); //输出
  bool BuildCNode(const CNodePtr &node, mind_ir::GraphProto *const graph_proto); //c节点
  std::string BuildInputNode(const AnfNodePtr &node, mind_ir::GraphProto *const graph_proto); //输入节点
//判断是否set函数
...
//get函数
 ...
 private:
  ...
  ModelProtoPtr model_; //创建名为model的ModelProto空间
  mind_ir::NodeProto *last_node_{nullptr};
  std::list<FuncGraphPtr> todo_; //链表
...
bool IrExportBuilder::SetAbstractFuncToAttributeProto(const abstract::AbstractBasePtr &abstract,
                                                      mind_ir::AttributeProto *const attr_proto) { 
  MS_EXCEPTION_IF_NULL(abstract); //捕捉是否是空指针异常
  MS_EXCEPTION_IF_NULL(attr_proto);
  if (abstract->isa<abstract::FuncGraphAbstractClosure>()) { //如果是函数图抽象闭包
    attr_proto->set_type(mind_ir::AttributeProto_AttributeType_FUNCGRAPHCLOSURE);//设置类型
    auto func_name = abstract->cast<abstract::FuncGraphAbstractClosurePtr>()->func_graph()->ToString();
    attr_proto->set_s(func_name); 
  } else if (abstract->isa<abstract::PrimitiveAbstractClosure>()) { //如果是原始抽象闭包
    attr_proto->set_type(mind_ir::AttributeProto_AttributeType_PRIMITIVECLOSURE); //设置类型
    auto prim = abstract->cast<abstract::PrimitiveAbstractClosurePtr>()->prim();
    attr_proto->set_s(GetPrimitiveUniqueName(prim)); 
  } else if (abstract->isa<abstract::PartialAbstractClosure>()) { //如果是部分抽象闭包
    ...
    MS_EXCEPTION_IF_NULL(node_ptr); //判断是否为空指针
    attr_proto->set_s(GetUniqueNodeName(node_ptr));
  } else if (abstract->isa<abstract::AbstractFuncUnion>()) { //如果是抽象函数并集
    ...
    auto visit_func = [this, &attr_proto](const abstract::AbstractFuncAtomPtr &poss) {
      ...
      if (!this->SetAbstractFuncToAttributeProto(poss, element_attr_proto)) {
        MS_LOG(EXCEPTION) << "Set union function abstract to proto error." << poss->ToString(); //抛出异常
      }
    };
    abstract->cast<abstract::AbstractFunctionPtr>()->Visit(visit_func);
  } else {
    MS_LOG(ERROR) << "The parameter abstract is not an abstractFunction: " << abstract->ToString(); //抛出错误
    return false;
  }
  return true;
}
...
std::string IrExportBuilder::GetPrimitiveUniqueName(const PrimitivePtr &primitive_ptr) {
  ...
  if (it != primitive_name_map_.end()) {
    return it->second; //it不等于primitive_name_map_.end()就指向下一个
  }
  auto answer = primitive_ptr->name() + ":" + std::to_string(GetUniqueID()); //将name和ID存在answer里
  primitive_name_map_[primitive_ptr] = answer; //再存到原始数据指针里
  return answer;
}

bool IrExportBuilder::BuildPrimitives() { //创建基本数据
  for (auto it = primitive_name_map_.begin(); it != primitive_name_map_.end(); ++it) { //遍历
    ...
    for (const auto &attr : prim->attrs()) { //遍历atrrs和基本属性
      ...
      if (!SetValueToAttributeProto(attr_value, attr_proto)) {
        MS_LOG(ERROR) << "Set value to AttributeProto failed."; //抛出错误
        return false;
      }
    }  // Loop of attrs
  }    // Loop of primitives
...
std::string IrExporter::GetDumpString(const FuncGraphPtr &func_graph) {
  ...
  if (dump_proto == nullptr) {
    MS_LOG(EXCEPTION) << "Get dump proto for graph " << func_graph->ToString() << " failed."; //空指针抛出异常
  }
...
ModelProtoPtr IrExporter::GetDumpProto(const FuncGraphPtr &func_graph, const FuncGraphPtr &param_layout_fg) {
  if ((builder_ == nullptr) || (func_graph == nullptr)) {
    MS_LOG(EXCEPTION) << "Input params is null."; //空指针抛出异常
  }

  // Export model info 导出模型信息
  builder_->BuildModelInfo();

  // Export model and return string 导出模型和返回字符串
  if (!builder_->BuildModel(func_graph)) {
    return nullptr;
  }

#ifndef MINDIR_EXPORT_TENSOR_LAYOUT_CLIP
  // Export layout information 导出布局信息
  if (param_layout_fg) {
...
std::string IrExportBuilder::GetProtoString() const {
  MS_LOG(DEBUG) << "BuildModel complete!"; //建立model完成
  return model_->SerializeAsString();
}

void IrExportBuilder::BuildModelInfo() { //利用set函数建立model中的基本信息
...
#ifndef MINDIR_EXPORT_TENSOR_LAYOUT_CLIP //防止二次定义
void IrExportBuilder::BuildLayout(const FuncGraphPtr &func_graph) { //设置布局
  ...
  for (auto para : graph_params) {
    ...
    if (tensor_layout == nullptr) {
      ...
      mind_ir::LayoutProto *layoutProto = parallel_proto->add_layout(); //tensor_layout不为空，则添加布局

      // Get all the information for layout 获取所有布局信息
      ...
      // Save all information to Layout Proto 将信息保存到Layou Proto中
...
bool IrExportBuilder::BuildModel(const FuncGraphPtr &func_graph) {
  ...
  graph_proto->set_name(func_graph->ToString()); //用set函数设置信息
  graph_proto->set_bprop_hash(func_graph->bprop_hash());
  todo_.clear(); //清空
  nodeName_.clear();
  primitive_name_map_.clear();
  // Build the main funcGraph 建立主要funcGraph
  (void)nodeName_.insert(func_graph->ToString());
  top_graph = true;
  if (!BuildFuncGraph(func_graph, graph_proto)) {
    MS_LOG(ERROR) << "Build func_graph " << func_graph->ToString() << " failed."; //抛出错误
    return false;
  }

  // Build child funcGraphs 建立子funcGraph
  ...
  while (!todo_.empty()) {
    ...
    if (graphVisited.count(fg) > 0) {
      continue; //若满足条件，则跳过该语句执行下一条
    }
    if (nodeName_.count(fg->ToString()) > 0) {
      MS_LOG(ERROR) << "There is a duplicate name: " << fg->ToString(); //当名称重复时抛出错误
      return false;
    }
    ...
    if (!BuildFuncGraph(fg, graph)) {
      MS_LOG(ERROR) << "Build func_graph " << fg->ToString() << " failed."; //抛出错误
      return false;
    }
  }
...
bool IrExportBuilder::BuildFuncGraph(const FuncGraphPtr &func_graph, mind_ir::GraphProto *const graph_proto) {
  // Export funcGraph name.到处funcGraph的名字
  graph_proto->set_name(func_graph->ToString());
  // Export parameters 导出参数
  // 1. parameters should be mapped to ValueInfoProto 参数应映射到ValueInfoProto
  // 2. parameters with default value should be mapped to Initializer 省略残缺的参数应映射到初始值设定项
  if (!BuildParameters(func_graph, graph_proto)) {
    MS_LOG(ERROR) << "Build parameters failed."; //抛出错误创建参数失败
    return false;
  }

  // Export graph attributes 导出图形属性
  if (!BuildFuncGraphAttrs(func_graph, graph_proto)) {
    MS_LOG(ERROR) << "Build attributes for graph failed.";
    return false;
  }

  // Export operator nodes(include output) 导出运算符节点（包括输出）
...
bool IrExportBuilder::BuildFuncGraphAttrs(const FuncGraphPtr &func_graph, mind_ir::GraphProto *const graph_proto) {
  ...
  for (auto attr : func_graph->attrs()) {
    MS_LOG(DEBUG) << "attr: " << attr.first << " " << attr.second->DumpText() << " " << attr.second->type_name();
    auto iter = g_export_attr_blacklist.find(attr.first);
    if (iter != g_export_attr_blacklist.end()) { //遍历
      continue;
    }
    mind_ir::AttributeProto *attr_proto = graph_proto->add_attribute();
    attr_proto->set_name(attr.first); //设置名字
    if (!SetValueToAttributeProto(attr.second, attr_proto)) {
      MS_LOG(ERROR) << "Set value to AttributeProto for GraphProto failed."; //将GraphProto的值设置为AttributeProto失败
      return false;
    }
  }
  return true;
...
bool IrExportBuilder::BuildParameters(const FuncGraphPtr &func_graph, mind_ir::GraphProto *const graph_proto) {
  ...
  for (auto &item : func_graph->parameters()) {
    ...
    auto param = item->cast<ParameterPtr>(); //将cast<ParameterPtr>()赋给param
    if (param == nullptr) {
      MS_LOG(ERROR) << "Parameter: '" << item->ToString() << "' could not cast to parameter."; //如果为空就报错
      return false;
    }
    std::string param_name = GetUniqueNodeName(param); //用get函数提出param的name
    if (top_graph && param->has_default()) {
      MS_LOG(DEBUG) << "Parameter: '" << item->DebugString();
      mind_ir::TensorProto *parameter_proto = graph_proto->add_parameter(); //用add函数将parameter存入张量中
      parameter_proto->set_name(param_name);
      if (!SetParamToTensorProto(param, parameter_proto)) {
        MS_LOG(ERROR) << "Set parameter " << param->DebugString() << " to TensorProto failed."; //报错，将parameter存入张量失败
        return false;
      }
    } else {
      mind_ir::ValueInfoProto *input_proto = graph_proto->add_input();
      input_proto->set_name(param_name); //设置名称
      if (!SetValueInfoProto(param, input_proto)) {
        MS_LOG(ERROR) << "Set parameter " << param->DebugString() << " to TensorProto failed."; //报错
        return false;
      }
    }
    if (nodeName_.count(param_name) > 0) {
      MS_LOG(ERROR) << "parameter name is duplicate:" << param_name; //参数名称重复
      return false;
    }
    (void)nodeName_.insert(param_name); //插入
  }
  return true;
}

mind_ir::TensorProto_DataType IrExportBuilder::GetMindirDataType(TypeId type_id) { //获取Mindir数据类型
  auto iter = g_data_type_map.find(type_id);
  if (iter == g_data_type_map.end()) { 
    MS_LOG(ERROR) << "Convert type error, unsupported type! " << type_id; //如果iter是g_data_type_map的结尾，则转换类型错误，该类型不支持
    return mind_ir::TensorProto_DataType_UNDEFINED;
  }
  return iter->second;
}

mind_ir::TensorProto_DataType IrExportBuilder::GetMindirDataBitsIntType(int bits) { //获取Int类型的Mindir数据
  auto iter = g_data_bits_int_map.find(bits);
  if (iter == g_data_bits_int_map.end()) {
    MS_LOG(ERROR) << "Convert bits int error, unsupported bits! " << bits; //转换错误
    return mind_ir::TensorProto_DataType_UNDEFINED;
  }
  return iter->second;
}

mind_ir::TensorProto_DataType IrExportBuilder::GetMindirDataBitsUIntType(int bits) { //获取UInt类型的Mindir数据位
  auto iter = g_data_bits_uint_map.find(bits);
  if (iter == g_data_bits_uint_map.end()) {
    MS_LOG(ERROR) << "Convert bits uint error, unsupported bits! " << bits; //转换错误
    return mind_ir::TensorProto_DataType_UNDEFINED;
  }
  return iter->second;
}

mind_ir::TensorProto_DataType IrExportBuilder::GetMindirDataBitsFloatType(int bits) { //获取Float类型的Mindir数据位
  auto iter = g_data_bits_float_map.find(bits);
  if (iter == g_data_bits_float_map.end()) {
    MS_LOG(ERROR) << "Convert bits float error, unsupported bits! " << bits; //转换错误
    return mind_ir::TensorProto_DataType_UNDEFINED;
  }
  return iter->second;
}
//判断是否设置ValueInfoProto
bool IrExportBuilder::SetValueInfoProto(const AnfNodePtr &node, mind_ir::ValueInfoProto *const value_proto) {
  if (node == nullptr || value_proto == nullptr) {
    MS_LOG(EXCEPTION) << "AnfNode or ValueInfo is null!"; //空指针异常
  }
  MS_LOG(DEBUG) << "SetValueInfoProto: " << node->DebugString(); //调试
  ...
  if (type->isa<TensorType>() && shape->isa<abstract::Shape>()) {
    ...
    if (!SetTensorProto(node->abstract(), tensor_proto)) {
      return false; //如果未设置TensorProto，则返回false
    }
  } else if (type->isa<Tuple>()) {
    mind_ir::AttributeProto *attribute = value_proto->mutable_attr_info();
    if (!SetAbstractToNodeProto(node->abstract(), attribute)) {
      MS_LOG(ERROR) << "Set shape to Proto for " << node->DebugString() << " failed."; //设置形状报错
      return false;
    }
    attribute->set_name("shape"); //设置名称
  } else {
    value_proto->set_denotation(type->type_name());
  }
  MS_LOG(DEBUG) << "Value type: " << type->type_name(); //调试
  return true;
}
//判断是否将张量设置为属性Proto
bool IrExportBuilder::SetTensorToAttributeProto(const ValuePtr &value, mind_ir::AttributeProto *const attr_proto) {
  if (value == nullptr || attr_proto == nullptr) {
    MS_LOG(EXCEPTION) << "ValuePtr or AttributeProto is null!"; //空指针异常
  }
  attr_proto->set_type(mind_ir::AttributeProto_AttributeType_TENSORS); //设置类型
  mind_ir::TensorProto *tensor_proto = attr_proto->add_tensors(); //添加张量
  tensor_proto->set_name("value0"); //设置名称
  auto data = value->cast<tensor::TensorPtr>();
  MS_EXCEPTION_IF_NULL(data); //判断是否为空
  tensor_proto->set_raw_data(data->data_c(), static_cast<size_t>(data->data().nbytes()));  //设置原始数据
  auto dtype = data->data_type(); //类型
  auto shape = data->shape_c(); //形状
  auto data_type = GetMindirDataType(dtype); //数据类型
  if (data_type == mind_ir::TensorProto_DataType_UNDEFINED) {
    return false; //如果数据类型和未定义数据类型相同，则返回false
  }
...
bool IrExportBuilder::SetCSRTensorToProto(const AbstractBasePtr &abstract, mind_ir::AttributeProto *const attr_proto) {
  ...
  MS_EXCEPTION_IF_NULL(csr_tensor_abs); //判断是否为空
  attr_proto->set_type(mind_ir::AttributeProto_AttributeType_CSR_TENSOR); //设置类型
  (void)SetTensorProto(csr_tensor_abs->indptr(), attr_proto->add_tensors()); //设置张量原型
...
bool IrExportBuilder::SetTensorProto(const AbstractBasePtr &abstract, mind_ir::TensorProto *const tensor_proto) {
  auto type = abstract->BuildType(); //创建类型
  auto shape = abstract->BuildShape(); //创建形状
  if (!type->isa<TensorType>() || !shape->isa<abstract::Shape>()) {
    MS_LOG(ERROR) << "Type or shape is not supported! " << type->ToString(); //报错，类型或形状错误
    return false;
  }
  auto tensor = type->cast<TensorTypePtr>(); //张量类型指针
  auto tensor_shape = shape->cast<abstract::ShapePtr>(); //形状指针
  const auto &dims = tensor_shape->shape(); //形状常量
  auto data_type = GetMindirDataType(tensor->element()->type_id()); //获取Mindir数据类型
  if (data_type == mind_ir::TensorProto_DataType_UNDEFINED) {
    return false; //如果数据类型和未定义数据类型相同，则返回false
  }
  tensor_proto->set_data_type(data_type);
  ...
  if (tensor_shape->IsDynamic()) {
    auto min_shape = tensor_shape->min_shape(); //最小形状
    auto max_shape = tensor_shape->max_shape(); //最大形状
  ...
  if (abs_ref == nullptr) {
    MS_LOG(ERROR) << "The abstract " << abstract->ToString() << " should be AbstractRef."; //抽象引用
    return false;
  }
  auto ref_key_value = abs_ref->ref_key_value();
  if (ref_key_value == nullptr) {
    MS_LOG(INFO) << "The ref_key_value of abstract ref " << abstract->ToString() << " is nullptr"; //空指针
    return true;
  }
...
bool IrExportBuilder::SetParamToTensorProto(const ParameterPtr &param, mind_ir::TensorProto *const tensor_proto) {
  if (param == nullptr || tensor_proto == nullptr) {
    MS_LOG(EXCEPTION) << "Parameter or TensorProto is null!"; //空指针异常
  }
  MS_LOG(DEBUG) << "SetParamToTensorProto: " << param->DebugString(); //调试
...
bool IrExportBuilder::BuildNodes(const FuncGraphPtr &func_graph, mind_ir::GraphProto *const graph_proto) {
  ...
  for (const AnfNodePtr &node : nodes) {
    MS_EXCEPTION_IF_NULL(node); //排除空指针
    if (!node->isa<CNode>()) { 
      MS_LOG(DEBUG) << "Node: '" << node->ToString() << "' is not cnode";
      continue;//不是cnode，执行下面语句
    }
    auto cnode = node->cast<CNodePtr>();
    if (cnode == func_graph->get_return()) {
      if (!BuildOutput(cnode, graph_proto)) {
        MS_LOG(ERROR) << "Build output for graph " << func_graph->ToString() << " failed."; //图形的生成输出失败
        return false;
      }
    } else {
      if (!BuildCNode(cnode, graph_proto)) {
        MS_LOG(ERROR) << "Build proto for cnode " << cnode->DebugString() << " failed."; //建立节点失败
        return false;
      }
    }
  }
...
bool IrExportBuilder::BuildOutput(const CNodePtr &node, mind_ir::GraphProto *const graph_proto) {
  ...
  if (node->size() != OutputSize) {
    MS_LOG(ERROR) << "Number of inputs of return node is not equal to 2."; //返回节点的输入数不等于2
    return false;
  }
  AnfNodePtr arg = node->input(1);
  std::string node_name = BuildInputNode(arg, graph_proto);
  if (node_name.empty()) { 
    MS_LOG(ERROR) << "Build input node failed for arg " << arg->DebugString(); //若为空则报错：参数生成输入节点失败
    return false;
  }
...
std::string IrExportBuilder::GetOpTypeName(const AnfNodePtr &node) { //获取OpTypeName
  ...
  if (IsValueNode<Primitive>(node)) {
    ...
    if (nodeName_.count(nodeName) == 0) {
      MS_LOG(ERROR) << "There is not the name: " << nodeName; //若为0.则没有名字
      return "";
    }
  } else {
...
bool IrExportBuilder::SetAbstractToNodeProto(const AbstractBasePtr &abs, mind_ir::AttributeProto *const attr_proto) {
  ...
  if (type->isa<Tuple>()) {
    attr_proto->set_type(mind_ir::AttributeProto_AttributeType_TUPLE); //设置类型
...
bool IrExportBuilder::SetAbstractToNodeProto(const CNodePtr &node, mind_ir::NodeProto *const node_proto) {
  // Get shape of cnode 获得cnode的形状
  // 1. need to get shape from tuple element 需要从元组元素获取形状
  // 2. save shape in TensorProto 在TensorProto中保存形状
...
bool IrExportBuilder::BuildCNode(const CNodePtr &node, mind_ir::GraphProto *const graph_proto) {
  ...
  if (inputs_size < 1) {
    MS_LOG(ERROR) << "Inputs of node " << node->DebugString() << " is empty"; //报错：节点输入为空
    return false;
  }

  // Need to build input node before dealing with cnode 在处理cnode之前需要构建输入节点
  std::vector<string> input_names;
  for (size_t i = 1; i < inputs_size; i++) {
    auto input = node->input(i);
    std::string node_name = BuildInputNode(input, graph_proto);
    if (node_name.empty()) {
      MS_LOG(ERROR) << "Build input node for " << input->DebugString() << " failed."; //报错构建输入节点失败
      return false;
    }
  ...
  // Maybe Tensor or Function or nullptr 可能是张量、函数或空指针
...
std::string IrExportBuilder::BuildInputNode(const AnfNodePtr &node, mind_ir::GraphProto *const graph_proto) {
  // Return the NodeName that the node has been processed.返回节点已处理的节点名。
  ...
  // FuncGraph will be added to functions and the input name is the function name. FuncGraph将添加到函数中，输入名称为函数名。
  ...
  if (node->isa<ValueNode>()) {
    ...
    // When node input is a ValueNode, need to create a Constant Node 当节点输入是ValueNode时，需要创建一个常量节点
    mind_ir::NodeProto *node_proto = graph_proto->add_node();
...
std::string IrExportBuilder::GetUniqueNodeName(const AnfNodePtr &node) {
  // Naming anfnode 命名节点
  // 1. parameter is unique in one func_graph 参数在一个func_
  // 2. cnode and valuenode may be reduplicative, so add index to identify. cnode和valuenode可能是重复的，因此添加索引以识别
  auto iter = node_name_map_.find(node);
  if (iter != node_name_map_.end()) {
    return iter->second;
  } else {
...
bool IrExportBuilder::SetAttributeProto(const AnfNodePtr &node, mind_ir::NodeProto *const node_proto) {
  if (node == nullptr || node_proto == nullptr) {
    MS_LOG(EXCEPTION) << "AnfNode or NodeProto is null!"; //若为空指针则抛出异常
  }
  auto value_node = node->cast<ValueNodePtr>();
  MS_EXCEPTION_IF_NULL(value_node);//排除空指针
  auto value = value_node->value();
  node_proto->set_op_type("Constant"); //设置op类型
  mind_ir::AttributeProto *attr_proto = node_proto->add_attribute();
  attr_proto->set_name("value"); //设置名称
  MS_LOG(DEBUG) << "Set Constant attribute: " << value->ToString(); //调试
...
bool IrExportBuilder::SetTypeToAttributeProto(const ValuePtr &value, mind_ir::AttributeProto *const attr_proto) {
  if (value == nullptr || attr_proto == nullptr) {
    MS_LOG(EXCEPTION) << "ValuePtr or AttributeProto is null!"; //若为空指针则抛出异常
  }
  attr_proto->set_type(mind_ir::AttributeProto_AttributeType_TENSORS); //设置类型
  mind_ir::TensorProto *tensor_proto = attr_proto->add_tensors(); //添加张量
  //判断int型、uint型、float型、bool型、tensor型
  if (value->isa<Int>()) {
    tensor_proto->set_name("value0"); //设置名称
    auto int_value = value->cast<IntPtr>();
    auto data_type = GetMindirDataBitsIntType(int_value->nbits());
    if (data_type == mind_ir::TensorProto_DataType_UNDEFINED) { //如果数据类型未定义，则返回false
      return false;
    }
    tensor_proto->set_data_type(data_type); 
    ...
    MS_LOG(EXCEPTION) << "Unsupported type: " << value->type_name(); //若都不是，则异常：没有支持的类型
    return false;
  }
  return true;
}
...
bool IrExportBuilder::SetValueToAttributeProto(const ValuePtr &value, mind_ir::AttributeProto *const attr_proto) {
  if (value == nullptr || attr_proto == nullptr) {
    MS_LOG(EXCEPTION) << "ValuePtr or AttributeProto is null!"; //空指针
  }
  //可能是stringimm或scalar、number或tensortype、ValueSequence、Tensor、None、Monad、IOMonad
  if (value->isa<StringImm>() || value->isa<Scalar>()) {
    ...
    if (!SetSequenceToAttributeProto(value->cast<ValueSequencePtr>(), attr_proto)) {
      MS_LOG(ERROR) << "Set sequence to AttributeProto failed."; //报错：将序列设置为AttributeProto失败
      return false;
    }
    MS_LOG(DEBUG) << "Attr string: " << value->type_name(); //调试
    ...
    if (value->isa<UMonad>()) {
      ...
      return false; //不支持的Monad类型，返回false
    }
  } else {
    MS_LOG(ERROR) << "Unsupported type: " << value->type_name(); //报错：没有支持的类型，返回false
    return false;
  }
  return true;
}
...
bool IrExportBuilder::SetScalarToAttributeProto_ir(const ValuePtr &value, mind_ir::AttributeProto *const attr_proto) {
  ...
  //可能是StringImm、BoolImm、Int、FP32Imm、FP64Imm
  if (value->isa<StringImm>()) {
    ...
    return false; //不支持的scalar类型，返回false
  }
  return true;
}
...
bool IrExportBuilder::SetScalarToAttributeProtoForInt_ir(const ValuePtr &value,
                                                         mind_ir::AttributeProto *const attr_proto) {
  //可能是Int8Imm、Int16Imm、Int32Imm、Int64Imm、UInt8Imm、UInt16Imm、UInt32Imm、UInt64Imm
...
bool IrExportBuilder::SetTypeToAttributeProto_irs(const ValuePtr &value, mind_ir::AttributeProto *const attr_proto) {
  if (attr_proto == nullptr) {
    MS_LOG(EXCEPTION) << "AttributeProto is null!";//空指针
  }
  //可能是Int、Float、UInt、Bool、Tensor
  if (value->isa<Int>()) {
    attr_proto->set_type(mind_ir::AttributeProto_AttributeType_TENSORS);
    mind_ir::TensorProto *tensor_proto = attr_proto->add_tensors();
    auto int_value = value->cast<IntPtr>();
...
bool IrExportBuilder::SetScalarToAttributeProto_irs(const ValuePtr &value, mind_ir::AttributeProto *const attr_proto) {
  ...
  可能是StringImm、BoolImm、FP32Imm、FP64Imm
  if (value->isa<StringImm>()) {
    attr_proto->set_type(mind_ir::AttributeProto_AttributeType_STRING);
...
//判断是否将将Seq Elem设置为AttributeProto
bool IrExportBuilder::SetSeqElemToAttributeProto(const ValuePtr &value, mind_ir::AttributeProto *const attr_proto) {
  if (value == nullptr) {
    MS_LOG(ERROR) << "Value is nullptr";//空指针
    return false;
  }
  if (value->isa<StringImm>() || value->isa<Scalar>()) {
    return SetScalarToAttributeProto_irs(value, attr_proto); //若为StringImm或Scalar，则返回SetScalarToAttributeProto_irs(value, attr_proto)
  }
  return SetTypeToAttributeProto_irs(value, attr_proto);
}
//判断是否将序列设置为AttributeProto
bool IrExportBuilder::SetSequenceToAttributeProto(const ValueSequencePtr &value,
                                                  mind_ir::AttributeProto *const attr_proto) {
  if (value == nullptr || attr_proto == nullptr) {
    MS_LOG(EXCEPTION) << "ValueSequencePtr or AttributeProto is null!"; //空
  }
  //可能是ValueTuple、ValueList
  if (value->isa<ValueTuple>()) {
    ...
    MS_LOG(EXCEPTION) << "The sequance value should be ValueTuple or ValueList, but it is " << value->ToString(); //sequance值应该是ValueTuple或ValueList
  }
  auto value_sequence = value->cast<ValueSequencePtr>();
  MS_EXCEPTION_IF_NULL(value_sequence); //排除空指针
  const auto &values = value_sequence->value();
  if (values.empty()) {
    MS_LOG(DEBUG) << "SetSequenceToAttributeProto sequence size is 0";  //如果为空，则调试SetSequenceToAttributeProto序列大小为0
    return true;
  }
  for (const auto &item : values) {
    mind_ir::AttributeProto *attr_values = attr_proto->add_values();
    MS_EXCEPTION_IF_NULL(item); //排除空指针 
...
std::string GetBinaryProtoString(const FuncGraphPtr &func_graph) {
  ...
  if (builder == nullptr) {
    MS_LOG(ERROR) << "Create ir exporter failed!"; //若为空则创建ir导出器失败
    return "";
  }
